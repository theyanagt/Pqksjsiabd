...
`${htki} KESEHATAN RENDAH ${htka}`,
`kesehatan kamu di bawah 80 ! 
tolong sembuhkan dulu Baru Adventure.`.trim(), 'lowhealth', [
[`ʜᴇᴀʟ ❤`, `${usedPrefix}heal`]
], m)
    if (new Date - user.lastadventure <= cooldown) return conn.sendButton(m.chat, 
`${htki} TENANG ${htka}`,
`Kamu Sudah *Berpetualang*, Tolong Tunggu sampai Tenang Selesai.

⏱️ ${timers.toTimeString()}`.trim(), 'cooldown', [
[`ɪɴᴠᴇɴᴛᴏʀʏ`, `${usedPrefix}inventory`],
[`ᴅᴀɪʟʏ`, `${usedPrefix}daily`]
], m)
    const rewards = reward(user)
    let text = `🔖 BERPETUALANG KE *${kt[1][0].name}*
...
handler.tags = ['rpg']
handler.command = /^ber(petua(lang?)?)?$/i
